...
def compute_result(voters, voter_record, x):
	cnt = 0
	tot = 0
	for z in voters:
...
def clf_party(voters, vote_property, voter_record):
	...
	cnt = 0
	print voters
	for vote in vote_property.keys():
		tmp = compute_result(voters, voter_record, vote)
		if (tmp != -1):
			cnt = cnt + 1
...
def predict(vote, vote_property_x, voters, vote_property, voter_record):
	...
	print V[1,:]
	for i in range(m):
		if (abs(V[0,i]) > abs(V[1,i])):
			vote_partyA.append(voters[i])
	
	clf = clf_party(vote_partyA, vote_property, voter_record)
	if (clf == 0 or clf == 1):
		tmp = clf
	else:
		tmp = clf.predict(vote_property_x)
...
def cross_validation(vote_property, vote_record, voter_record):
	...
	for x in vote_record.keys():
		...
		real = compute_result(voters, voter_record, x) 
